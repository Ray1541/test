...
package com.jiang.test3_0.textwarrior.common;
...
public abstract class ColorScheme
{
	...
	private HashMap<Colorable, Integer> generateDefaultColors()
	{
		...
		colors.put(Colorable.SELECTION_BACKGROUND, select_bg);//选择文本的背景色
		...
		colors.put(Colorable.LINE_HIGHLIGHT, 0x259F79EE);//选中时的颜色

		colors.put(Colorable.NON_PRINTING_GLYPH, non_printing_glyph);//行号
		colors.put(Colorable.COMMENT, OLIVE_GREEN); //注释
		colors.put(Colorable.KEYWORD, Key); //关键字
		...
		colors.put(Colorable.SECONDARY, 0xffFFA500);//宏定义
		return colors;
	}
	...
	private static final int Key = 0xFFCD3278;
	private static final int select_bg = 0xFFBFEFFF;
	private static final int non_printing_glyph = 0xff668B8B;
}
